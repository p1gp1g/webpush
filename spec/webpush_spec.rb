...
describe LegacyWebpush do
  it 'has a version number' do
    expect(LegacyWebpush::VERSION).not_to be nil
  end
  ...
  shared_examples 'web push protocol standard error handling' do
    ...
    it 'raises PayloadTooLarge if the API returns a 413 Error' do
      ...
      expect { subject }.to raise_error(LegacyWebpush::PayloadTooLarge)
    end

    it 'raises TooManyRequests if the API returns a 429 Error' do
      stub_request(:post, expected_endpoint).
          to_return(status: 429, body: "", headers: {})
      expect { subject }.to raise_error(LegacyWebpush::TooManyRequests)
    end

    it 'raises PushServiceError if the API returns a 5xx Error' do
      stub_request(:post, expected_endpoint).
          to_return(status: 500, body: "", headers: {})
      expect { subject }.to raise_error(LegacyWebpush::PushServiceError)
    ...
    it 'raises ResponseError for unsuccessful status code by default' do
      ...
      expect { subject }.to raise_error(LegacyWebpush::ResponseError)
    end
    ...
    it 'supplies the original status code on the ResponseError' do
      ...
      expect { subject }.to raise_error { |error|
        expect(error).to be_a(LegacyWebpush::ResponseError)
  ...
  shared_examples 'request headers with VAPID' do
    ...
    before do
      allow(LegacyWebpush::Encryption).to receive(:encrypt).and_return(payload)
      allow(JWT).to receive(:encode).and_return('jwt.encoded.payload')
    end

    subject { LegacyWebpush.payload_send(
      message: message,
      endpoint: endpoint,
      p256dh: p256dh,
      auth: auth,
    ...
    it 'calls the relevant service with the correct headers' do
      expect(LegacyWebpush::Encryption).to receive(:encrypt).and_return(payload)
    ...
    it 'message is optional' do
      expect(LegacyWebpush::Encryption).to_not receive(:encrypt)
      ...
      LegacyWebpush.payload_send(endpoint: endpoint)
    end
    ...
    it 'vapid options are optional' do
      ...
      result = LegacyWebpush.payload_send(
        message: message,
        endpoint: endpoint,
        p256dh: p256dh,
        auth: auth)
  ...
  context 'chrome GCM endpoint: request headers with GCM api key' do
    ...
    subject { LegacyWebpush.payload_send(message: message, endpoint: endpoint, p256dh: p256dh, auth: auth, api_key: "GCM_API_KEY") }
    ...
    it 'message and encryption keys are optional' do
      ...
      LegacyWebpush.payload_send(endpoint: endpoint, api_key: 'GCM_API_KEY')
    end
  end
end
